...
- COLOR CONVENTION (IMPORTANT): on these sheets the PACK count (bags) is written in BLUE or BLACK ink, while the COUNTED-IN reference is written in RED ink. So: blue/black numbers → "bags" (actual packed pieces); red numbers → "reference" (counted-in, for cross-check only). If a row has ONLY a red number and no blue/black number, that row has NO pack count → return bags = [] (empty) and reference = the red number. NEVER promote a red counted-in number into "bags". When a digit is ambiguous (e.g. 13 vs 73, 14 vs 19), prefer the reading whose color matches its role and whose value is consistent with neighbouring rows.
